...
def download_image(url, save_path, local_port=None, timeout=30):
    """下载图片到本地临时文件（用于sendMediaGroup的photo）"""
    try:
        opener = build_opener(local_port)
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 17_2 like Mac OS X) AppleWebKit/605.1.15"})
        resp = opener.open(req, timeout=timeout)
        data = resp.read()
...
def api_send_media_group(token, chat_id, photo_path, doc_path, caption="", local_port=None, timeout=120):
    """发送相册（sendMediaGroup）：一条消息包含图片+文件，文字做图片caption。
    media数组第1个是photo（带caption），第2个是document（py文件）。"""
    if not os.path.isfile(photo_path):
        return {"ok": False, "description": f"图片不存在: {photo_path}"}
    if not os.path.isfile(doc_path):
        return {"ok": False, "description": f"文件不存在: {doc_path}"}
    ...
    media = [
        ...
        {"type": "document", "media": "attach://doc1"},
    ]
    ...
    # media（JSON数组）
    body += f"--{boundary}\r\n".encode("utf-8")
    body += b'Content-Disposition: form-data; name="media"\r\n\r\n'
    body += media_json.encode("utf-8") + crlf

    # 图片文件
    with open(photo_path, "rb") as f:
        photo_data = f.read()
    body += f"--{boundary}\r\n".encode("utf-8")
    body += b'Content-Disposition: form-data; name="photo1"; filename="cover.jpg"\r\n'
    ...
    with open(doc_path, "rb") as f:
        doc_data = f.read()
    doc_filename_encoded = urllib.parse.quote(doc_filename)
    body += f"--{boundary}\r\n".encode("utf-8")
    body += f'Content-Disposition: form-data; name="doc1"; filename="{doc_filename_encoded}"\r\n'.encode("utf-8")
    body += b"Content-Type: application/octet-stream\r\n\r\n"
    body += doc_data + crlf
