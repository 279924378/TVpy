...
26. **任务结果汇总推送铁律（2026-09-14修订·分析结果不单独推群，汇总到最终文件caption一次性推送，含升级情况+核心价值观）**：执行tvbox-dev流水线时，**各阶段分析结果不单独推送群通知**，而是全部汇总到最终文件推送的caption中，一次性推送。**具体要求**：①**对话内进度更新**：各阶段（开始侦察/侦察完成/爬虫开发完成/仓库同步完成）只在豆包对话中给一句话进度更新，**不调用tg_pusher.py单独推群**；②**分析结果收集**：执行过程中必须收集以下分析结果，供最终caption使用——站点类型（苹果CMS/SPA/其他）、分类总数+完整分类名称列表、反爬机制（CF/WAF/无）、关键发现（播放地址来源/m3u8格式/特殊结构）、视频列表每页数量、自测检测结果（全接口通过/未成年剔除）；③**最终一次性推送（一条消息带出）**：全部完成后，**只推送一条消息**，使用 `sendDocument`（文件附件+thumbnail缩略图+caption文字），**整条任务只有这一条群消息**，图片、文字、文件全部在一条消息里，禁止分两条消息推送，禁止先发图片再发文件；④**caption必须含升级情况+核心价值观（2026-09-14新增，简短核心）**：caption末尾必须加入简短的升级情况介绍和核心价值观，格式如下（固定模板，简短不啰嗦）：
```
🔧 技能升级：数据库驱动·三语言规范融合·25源术持续更新
💎 核心价值观：铁律至上·质量第一·持续进化·道友至上
```
