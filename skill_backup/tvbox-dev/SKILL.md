...
29. **推送完成即清理铁律（2026-09-14新增，铁律级·掘成即清·不留痕迹）**：Spider开发完成、推送成功（TG群推送+坚果云上传+GitHub上传全部完成）后，**必须立即清理本地工作目录中的临时文件和爬虫记录，禁止文件堆积、禁止留痕**。**具体要求**：①**删除本地py文件**：推送成功后立即删除项目目录中的Spider py文件（如`/home/user/Doubao/chats/<session>/<站点名>.py`），该文件已上传至GitHub和坚果云，本地无需保留；②**删除侦察临时HTML文件**：删除侦察过程中保存的所有临时HTML文件（如`home.html`、`cat.html`、`play.html`、`detail.html`、`home1.html`、`cat1.html`等带编号的HTML文件），以及下载的临时封面图（`/tmp/*.jpg`、`/tmp/*.png`）；③**标记待掘队列已处理**：将该站点网址在待掘队列（tg_pending_urls.json / pending_urls表）中的状态标记为`done`（或删除该记录），避免重复爬取；④**禁止删除的内容**：GitHub仓库中的文件（已永久备份）、坚果云中的文件（已永久备份）、数据库中的记录（spider_registry Spider注册、push_history推送历史、operation_logs操作日志）、技能目录中的脚本和配置文件（tg_pusher.py、nutstore_uploader.py等）、用户明确要求保留的文件；⑤**清理时机**：必须在推送成功后**立即执行**（同一轮工具调用内完成），禁止拖延到下一轮、禁止忘记清理。**判定标准**：项目目录中无该站点的py文件、无侦察临时HTML文件、待掘队列中该网址状态为done（或已删除），三项全部满足才算本铁律执行完毕。
