...
26. **任务结果汇总推送铁律（2026-09-14修订·分析结果不单独推群，汇总到最终文件caption一次性推送）**：执行tvbox-dev流水线时，**各阶段分析结果不单独推送群通知**，而是全部汇总到最终文件推送的caption中，一次性推送。**具体要求**：①**对话内进度更新**：各阶段（开始侦察/侦察完成/爬虫开发完成/仓库同步完成）只在豆包对话中给一句话进度更新，**不调用tg_pusher.py单独推群**；②**分析结果收集**：执行过程中必须收集以下分析结果，供最终caption使用——站点类型（苹果CMS/SPA/其他）、分类总数+完整分类名称列表、反爬机制（CF/WAF/无）、关键发现（播放地址来源/m3u8格式/特殊结构）、视频列表每页数量、自测检测结果（全接口通过/未成年剔除）；③**最终一次性推送（一条消息带出）**：全部完成后，**只推送一条消息**，使用 `sendMediaGroup`（相册形式）包含两个media——第1个 `inputMediaPhoto`（网站首页随机视频封面图，**完整文字说明+全部分析结果+分类信息+上传状态+订阅地址作为这个图片的caption**），第2个 `inputMediaDocument`（py文件附件）；**整条任务只有这一条群消息**，图片、文字、文件全部在一个相册里，禁止分两条消息推送，禁止先发图片再发文件；④**禁止单独推群**：禁止在执行过程中调用tg_pusher.py推送任何阶段通知（开始侦察/侦察完成/开发完成等都不推），禁止先发文字再发文件（必须用sendDocument的caption合并）。**判定标准**：群里每次任务只有**1条新消息**（一个sendMediaGroup相册，含图片+文件，图片caption含完整分析结果和分类信息），缺少任何分析项或分成多条消息即视为本铁律未执行。
