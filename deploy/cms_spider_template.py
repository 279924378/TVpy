...
DEFAULT_MIRRORS = ("mirror1.com", "mirror2.com")
PAGE_SIZE = 20
DEFAULT_UA = (
...
class Spider:
    ...
    def _parse_categories(self, html_text):
        classes = []
        seen = set()
        for dl_m in re.finditer(r'<dl[^>]*>(.*?)</dl>', str(html_text or ""), re.DOTALL):
            dl_html = dl_m.group(1)
            parent_name = ""
            parent_id = ""
            dt_m = re.search(r'<dt[^>]*>\s*<a[^>]*href="[^"]*type/id/(\d+)\.html"[^>]*>([^<]+)</a>', dl_html)
            if dt_m:
                parent_id = dt_m.group(1)
                parent_name = dt_m.group(2).strip()
                if parent_id not in seen and parent_name and parent_name != "网站首页":
                    seen.add(parent_id)
                    classes.append({"type_id": parent_id, "type_name": parent_name})
            for dd_m in re.finditer(r'<dd[^>]*>\s*<a[^>]*href="[^"]*type/id/(\d+)\.html"[^>]*>([^<]+)</a>', dl_html):
                tid = dd_m.group(1)
                tname = dd_m.group(2).strip()
                if tid in seen or not tname or tname == "网站首页":
                    continue
                seen.add(tid)
                full_name = (parent_name + "-" + tname) if parent_name else tname
                classes.append({"type_id": tid, "type_name": full_name})
        if not classes:
            for m in re.finditer(r'<a[^>]*href="[^"]*type/id/(\d+)\.html"[^>]*>([^<]+)</a>', str(html_text or "")):
    ...
    def _parse_list(self, html_text):
        ...
        for m in re.finditer(
            r'<a[^>]*class="vtz9\s+thumbnail"[^>]*href="([^"]+)"[^>]*>.*?'
            r'<img[^>]*src="([^"]+)"[^>]*alt="([^"]*)"',
            str(html_text or ""), re.DOTALL
        ):
            detail_url, pic, name = m.group(1), m.group(2), m.group(3).strip()
            vid = self._extract_id(detail_url)
    ...
    def detailContent(self, ids):
        ...
        for source_id in id_list:
            ...
            if not vid or not re.search(r'\d', vid):
                ...
                continue
            detail_url = self.host + "/index.php/vod/detail/id/" + vid + ".html"
            try:
            ...
            pic_m = re.search(r'<div[^>]*class="detail-poster"[^>]*>.*?<img[^>]*src="([^"]+)"', html_text, re.DOTALL)
            pic = urljoin(self.host + "/", pic_m.group(1)) if pic_m else ""
            name_m = re.search(r'<li>片名：([^<]+)</li>', html_text)
            name = name_m.group(1).strip() if name_m else ""
            if not name:
                t_m = re.search(r'<title>([^<]+)</title>', html_text)
                name = t_m.group(1).split("--")[0].strip() if t_m else vid
            type_m = re.search(r'<li><label>类型：</label>([^<]+)</li>', html_text)
            vod_class = type_m.group(1).strip() if type_m else ""
            update_m = re.search(r'<li><label>更新：</label>([^<]+)</li>', html_text)
            remarks = update_m.group(1).strip() if update_m else ""
            source_names = []
            for sm in re.finditer(r'<ul[^>]*class="detail-tab[^"]*"[^>]*>.*?</ul>', html_text, re.DOTALL):
                for am in re.finditer(r'<li[^>]*>\s*<a[^>]*>([^<]+)</a>', sm.group(0)):
                    sn = am.group(1).strip()
                    if sn and sn not in source_names:
                        source_names.append(sn)
            if not source_names:
                source_names = ["默认线路"]
            play_lists = []
            for pane in re.finditer(r'<ul[^>]*class="detail-play-list[^"]*"[^>]*>(.*?)</ul>', html_text, re.DOTALL):
                eps = []
                for am in re.finditer(r'<li><a[^>]*href="([^"]+)"[^>]*title="([^"]*)"[^>]*>([^<]*)</a></li>', pane.group(1)):
                    play_url, ep_title, ep_text = am.group(1), am.group(2), am.group(3).strip()
                    if 'play' not in play_url:
                        continue
                    ep_name = ep_title if ep_title else (ep_text if ep_text else "正片")
                    eps.append(ep_name + "$" + play_url)
                if eps:
                    play_lists.append(eps)
            if not play_lists:
                play_lists = [[]]
            min_len = min(len(source_names), len(play_lists))
            source_names = source_names[:min_len] if min_len > 0 else ["默认线路"]
            play_lists = play_lists[:min_len] if min_len > 0 else [[]]
            vod = {
                "vod_id": vid, "vod_name": name, "vod_pic": pic, "vod_remarks": remarks,
                "vod_year": "", "vod_area": "", "vod_lang": "", "vod_actor": "",
                "vod_director": "", "vod_type": vod_class, "vod_content": name + " - " + vod_class,
                "vod_play_from": "$$$".join(source_names),
                "vod_play_url": "$$$".join("#".join(eps) for eps in play_lists),
            }
    ...
    def playerContent(self, flag, id, vipFlags=None):
        ...
        try:
            ...
            return {"parse": 0, "jx": 0, "playUrl": "", "url": "", "header": {}, "msg": _clean_text(exc)}
        real_url = ""
        m = re.search(r'player_aaaa\s*=\s*(\{[\s\S]+?\})\s*</script>', html_text)
        if m:
            try:
        ...
        if not real_url:
            ...
            if m3u8:
                real_url = m3u8.group(0)
        if not real_url:
            return {"parse": 0, "jx": 0, "playUrl": "", "url": "", "header": {}, "msg": "未解析到可播放的 m3u8"}
        # 铁律·广告拦截：m3u8地址走本地代理清洗（壳支持getProxyUrl时生效）
